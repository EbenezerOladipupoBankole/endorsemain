...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background font-sans">
      {/* Navigation */}
      <header className={`sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 transition-all duration-300 ${isScrolled ? "h-28" : "h-48"}`}>
        <div className="container flex h-full items-center justify-between px-4 md:px-6">
          <Logo className={`w-auto transition-all duration-300 ${isScrolled ? "h-24" : "h-44"}`} />
